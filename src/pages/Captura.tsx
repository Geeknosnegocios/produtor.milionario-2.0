...
const Captura = () => {
  ...
  const giftFiredRef = useRef(false);
  const giftStartRef = useRef<number>(Date.now());

  useEffect(() => { trackViewContent(); }, []);

  // Exit-intent: dispara o popup de presente quando o mouse sai pelo topo
  useEffect(() => {
    if (typeof window === "undefined") return;
    try { if (sessionStorage.getItem("captura-gift-dismissed") === "1") return; } catch { /* noop */ }
    giftStartRef.current = Date.now();
    const handle = (e: MouseEvent) => {
      if (giftFiredRef.current) return;
      if (giftOpen) return;
      const to = e.relatedTarget as Node | null;
      if (to !== null) return;
      if (e.clientY > 0) return;
      if (Date.now() - giftStartRef.current < 4000) return;
      giftFiredRef.current = true;
      setGiftOpen(true);
    };
  ...
  const closeGift = (next: boolean) => {
    setGiftOpen(next);
    if (!next) { try { sessionStorage.setItem("captura-gift-dismissed", "1"); } catch { /* noop */ } }
  };

  const claimGift = () => {
    try { trackLead("gift-exit-modal", 0); } catch { /* noop */ }
    window.open(GRUPO_URL, "_blank", "noopener,noreferrer");
    closeGift(false);
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      {/* Exit-intent · presente exclusivo do grupo */}
      <Dialog open={giftOpen} onOpenChange={closeGift}>
        <DialogContent className="max-w-md max-md:max-w-[95vw] max-h-[95vh] overflow-y-auto border border-green-500/40 bg-[#0E0F11] p-0 rounded-2xl shadow-[0_0_80px_rgba(34,197,94,0.4)]">
          ...
          <div className="relative px-5 sm:px-7 pt-7 pb-5 text-center">
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-[10px] sm:text-xs text-green-300 font-mono uppercase tracking-[0.18em] font-bold">Espera · presente pra você</span>
              <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
            </div>
